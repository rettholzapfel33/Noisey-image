...
from src.cae.src.dataloaderAE import preprocess_single
...
def main(config, image, size) -> None:
    ...
    detectImages(cfg, image, show_stats=size)

def detectImages(cfg: Namespace, image, show_stats=False) -> None:
...
if __name__ == "__main__":
    ...
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    detectImages(cfg, image, show_stats=args.size)
